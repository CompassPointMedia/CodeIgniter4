...
class SampleModuleRouting implements \CodeIgniter\Module\ModuleRoutingInterface {
    ...
    public function titleMatchSortOf($segment, $title){
        ...
        $changes = 7;
        $match_level = 1 - $lev / $changes;
        return $match_level > 0 ? $match_level : 0.0;
    }
    ...
    public function titleRenderMatchUri($uri, $record){
        ...
        foreach($segments as $segment){
            ...
            if($rid_token && is_numeric($segment)){
                $responseid = $segment;
            }else{
                //bad uri
                $rid_token = false;
            }
            if(strtolower($segment) === 'responseid') $rid_token = true;


            // Handle title match
            if($segment === $title){
                $title_match = 1;
            }else if($match_level = $this->titleMatchSortOf($segment, $title)){
                // perhaps we'll accept an approximate match
                $title_match = $match_level;
            }
        }
        if($title_match){
            if($responseid > 0.0) $this->params = [
                $record = array_merge($record, [ '_title_match' => $title_match]),
                $responseid
            ];
            return $record;
        }
